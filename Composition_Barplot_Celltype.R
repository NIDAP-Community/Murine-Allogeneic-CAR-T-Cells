...
bar_table <- table(so@meta.data$ProjectTILS_calls, so@meta.data$SCT_snn_res.0.4)

# For aggregated samples
counts_percentage_aggregate_df <- table(so@meta.data$ProjectTILS_calls, so@meta.data$SCT_snn_res.0.4)
...
g <- ggplot(counts_percentage_aggregate_df, aes(fill=CellType, y=`Proportion`, x=Sample)) + ggtitle("CellType Proportion in each Sample") + theme_classic() +
